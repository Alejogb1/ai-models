...
| 6 | F1 measures success | | | |
| 7 | Taxonomy is static | *(deferred — requires longitudinal logs)* | | |
| 8 | English-first | *(deferred — English-only for v1)* | | |
| 9 | Bottleneck is labels | *(deferred — depends on Exp 4 result)* | | |
| 10 | Octohedral is universal | *(deferred — depends on Exp 3 result)* | | |
| 11 | Raw text input | | | |
| 12 | Every query classified | | | |
